...
int main(int argc, char** argv){
	...
	//-------------------------------------------------------------------------------------------------------------------------------------------------------------
    double MinEnergy = 0, MaxEnergy = 5000;
	int nbinEnergyW = 50;
	...
	TH2F events_yz("events_yz","Eevents in detector coords.", (2398 - 2384)*100, 2384, 2398, (13.4-13.15)*1000, -13.4, -13.15);
   ...
   //energy vs momentum TProfile
	...
	TH1F stuck_primary_muminus_energy("stuck_primary_muminus_energy","Energy distribution of the stuck prim. #mu^{-}", nbinEnergy, 0, MaxEnergy);
	TH1F new_primary_muminus_energy("new_primary_muminus_energy","Energy distribution of the post- " + Tpost_vol + " new prim. #mu^{-}", nbinEnergy, 0, MaxEnergy);
    ...
    //crossed_primary_muminus_mom_energy.SetFillStyle(kSpring+8,0.3);
    //crossed_primary_muminus_energy.SetFillStyle(kGreen+2,0.3);
    //new_primary_muminus_energy.SetFillStyle(kRed,0.3);
    stuck_primary_muminus_energy.SetFillStyle(3023);
    
    
    //secondary
	TH1F crossed_secondary_muminus_mom_energy("crossed_secondary_muminus_mom_energy","Energy distribution of the  " + Tpost_vol + "-entering sec. #mu^{-}'s mom #mu^{-}", nbinEnergy, 0, MaxEnergy);
	TH1F crossed_secondary_muminus_energy("crossed_secondary_muminus_energy","Energy distribution of the  " + Tpost_vol + "-entering sec. #mu^{-}", nbinEnergy, 0, MaxEnergy);
	TH1F stuck_secondary_muminus_energy("stuck_secondary_muminus_energy","Energy distribution of the stuck sec. #mu^{-}", nbinEnergy, 0, MaxEnergy);
	TH1F new_secondary_muminus_energy("new_secondary_muminus_energy","Energy distribution of the post- " + Tpost_vol + " new sec. #mu^{-}", nbinEnergy, 0, MaxEnergy);
    ...
    stuck_secondary_muminus_energy.SetFillStyle(3023);
   ...
   //energy vs momentum TProfile
    ...
    //muon 
    //primary
	TH1F crossed_primary_muplus_mom_energy("crossed_primary_muplus_mom_energy","Energy distribution of the  " + Tpost_vol + "-entering #mu^{-}'s mom prim. #mu^{-}", nbinEnergy, 0, MaxEnergy);
	TH1F crossed_primary_muplus_energy("crossed_primary_muplus_energy","Energy distribution of the  " + Tpost_vol + "-entering prim. #mu^{-}", nbinEnergy, 0, MaxEnergy);
	TH1F stuck_primary_muplus_energy("stuck_primary_muplus_energy","Energy distribution of the stuck prim. #mu^{-}", nbinEnergy, 0, MaxEnergy);
	TH1F new_primary_muplus_energy("new_primary_muplus_energy","Energy distribution of the post- " + Tpost_vol + " new prim. #mu^{-}", nbinEnergy, 0, MaxEnergy);
    ...
    //secondary
	TH1F crossed_secondary_muplus_mom_energy("crossed_secondary_muplus_mom_energy","Energy distribution of the  " + Tpost_vol + "-entering #mu^{-}'s mom sec. #mu^{-}", nbinEnergy, 0, MaxEnergy);
	TH1F crossed_secondary_muplus_energy("crossed_secondary_muplus_energy","Energy distribution of the  " + Tpost_vol + "-entering sec. #mu^{-}", nbinEnergy, 0, MaxEnergy);
	TH1F stuck_secondary_muplus_energy("stuck_secondary_muplus_energy","Energy distribution of the stuck sec. #mu^{-}", nbinEnergy, 0, MaxEnergy);
	TH1F new_secondary_muplus_energy("new_secondary_muplus_energy","Energy distribution of the post- " + Tpost_vol + " new sec. #mu^{-}", nbinEnergy, 0, MaxEnergy);
	...
	TH1F crossed_piplus_mom_energy("crossed_piplus_mom_energy","Energy distribution of the  " + Tpost_vol + "-entering #piplus^{-}'s mom #piplus^{-}", nbinEnergy, 0, MaxEnergy);
	TH1F crossed_piplus_energy("crossed_piplus_energy","Energy distribution of the  " + Tpost_vol + "-entering #piplus^{-}", nbinEnergy, 0, MaxEnergy);
	TH1F stuck_piplus_energy("stuck_piplus_energy","Energy distribution of the stuck #piplus^{-}", nbinEnergy, 0, MaxEnergy);
	TH1F new_piplus_energy("new_piplus_energy","Energy distribution of the post- " + Tpost_vol + " new #piplus^{-}", nbinEnergy, 0, MaxEnergy);
	
    crossed_piplus_mom_energy.GetXaxis()->SetTitle("MeV");
    crossed_piplus_energy.GetXaxis()->SetTitle("MeV");
	new_piplus_energy.GetXaxis()->SetTitle("MeV");
	stuck_piplus_energy.GetXaxis()->SetTitle("MeV");
	crossed_piplus_mom_energy.GetYaxis()->SetTitleOffset(1.0);	
	crossed_piplus_mom_energy.GetXaxis()->SetTitleOffset(1.0);	
	crossed_piplus_mom_energy.GetYaxis()->SetTitleSize(0.05);	
	crossed_piplus_mom_energy.GetXaxis()->SetTitleSize(0.05);	
	crossed_piplus_energy.GetYaxis()->SetTitleOffset(1.0);	
	crossed_piplus_energy.GetXaxis()->SetTitleOffset(1.0);	
	crossed_piplus_energy.GetYaxis()->SetTitleSize(0.05);	
	crossed_piplus_energy.GetXaxis()->SetTitleSize(0.05);	
	stuck_piplus_energy.GetXaxis()->SetTitle("M (MeV/c^{2})");
	stuck_piplus_energy.GetYaxis()->SetTitleOffset(1.0);	
	stuck_piplus_energy.GetXaxis()->SetTitleOffset(1.0);	
	stuck_piplus_energy.GetYaxis()->SetTitleSize(0.05);	
	stuck_piplus_energy.GetXaxis()->SetTitleSize(0.05);	
	new_piplus_energy.GetYaxis()->SetTitleOffset(1.0);	
	new_piplus_energy.GetXaxis()->SetTitleOffset(1.0);	
	new_piplus_energy.GetYaxis()->SetTitleSize(0.05);	
	new_piplus_energy.GetXaxis()->SetTitleSize(0.05);	
	
    crossed_piplus_mom_energy.SetFillColorAlpha(kSpring+8,0.3);
    crossed_piplus_energy.SetFillColorAlpha(kGreen+2,0.3);
    new_piplus_energy.SetFillColorAlpha(kRed,0.3);
    ...
    new_piplus_energy.SetLineColor(kRed+1);
    ...
    for(int i=0; i < tree->GetEntries(); i++) 
    {
        ...
        for(int ip = 0; ip < StdHepN; ip++) 
        {
            ...
            if (StdHepPdg[ip] == mu_pdg)std::cout << "Muon's first mother:    " << my_pdg.GetName(StdHepPdg[StdHepFm[ip]]) << ", Last mother:   " << my_pdg.GetName(StdHepPdg[StdHepLm[ip]]) << "\n"; 
	...
	if (print3 == "-D")
	{
		...
		canvas.Update();
		stuck_primary_muminus_energy.Draw("same");
		canvas.Update();
        TLegend *mu_energy_prim_leg = new TLegend(0.1,0.7,0.48,0.9);
        mu_energy_prim_leg->SetHeader("#mu^{-}","C");
        mu_energy_prim_leg->AddEntry(&crossed_primary_muminus_energy,"crossed prim. #mu^{-}","f");
        mu_energy_prim_leg->AddEntry(&stuck_primary_muminus_energy,"stuck prim. #mu^{-} energy","f");
        mu_energy_prim_leg->Draw();


        canvas.cd(2);
		crossed_secondary_muminus_energy.Draw();
		canvas.Update();
		stuck_secondary_muminus_energy.Draw("same");
		canvas.Update();
        TLegend *mu_energy_sec_leg = new TLegend(0.1,0.7,0.48,0.9);
        mu_energy_sec_leg->SetHeader("#mu^{-}","C");
        mu_energy_sec_leg->AddEntry(&crossed_secondary_muminus_energy,"crossed sec. #mu^{-}","f");
        mu_energy_sec_leg->AddEntry(&stuck_secondary_muminus_energy,"stuck sec. #mu^{-} energy","f");
        mu_energy_sec_leg->Draw();
		...
		canvas.Update();
        TLegend *pi_energy_leg = new TLegend(0.1,0.7,0.48,0.9);
        pi_energy_leg->SetHeader("#pi^{-}","C");
        pi_energy_leg->AddEntry(&crossed_piminus_energy,"crossed #pi^{-}","f");
        pi_energy_leg->AddEntry(&stuck_piminus_energy,"stuck #pi^{-} energy","f");
        pi_energy_leg->Draw();
		//secondaries	

        
        
		TCanvas canvas2("canvas2","",0,0,900,900);
		//canvas2.Divide(2,1);
		//canvas2.cd(1);
		//muon_theta.Draw();
		//canvas2.cd(2);
