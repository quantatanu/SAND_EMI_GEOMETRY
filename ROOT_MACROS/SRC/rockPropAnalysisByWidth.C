...
int main(int argc, char** argv){
	...
	TH1F new_piminus_energy("new_piminus_energy","Energy distribution of the new #pi^{-}", nbinEnergy, 0, MaxEnergy);
	

	new_piminus_energy.GetXaxis()->SetTitle("E (MeV)");
	new_piminus_energy.GetYaxis()->SetTitleOffset(0.92);	
	new_piminus_energy.GetXaxis()->SetTitleOffset(0.92);	
	new_piminus_energy.GetYaxis()->SetTitleSize(0.06);	
	new_piminus_energy.GetXaxis()->SetTitleSize(0.06);	
	new_piminus_energy.GetYaxis()->SetLabelSize(0.06);	
	new_piminus_energy.GetXaxis()->SetLabelSize(0.06);	
    new_piminus_energy.SetFillColorAlpha(kMagenta,0.3);
	new_piminus_energy.SetLineColor(kMagenta+3);
	...
	TH1F new_piplus_energy("new_piplus_energy","Energy distribution of the new #pi^{+}", nbinEnergy, 0, MaxEnergy);


	new_piplus_energy.GetXaxis()->SetTitle("E (MeV)");
	new_piplus_energy.GetYaxis()->SetTitleOffset(0.92);	
	new_piplus_energy.GetXaxis()->SetTitleOffset(0.92);	
	new_piplus_energy.GetYaxis()->SetTitleSize(0.06);	
	new_piplus_energy.GetXaxis()->SetTitleSize(0.06);	
	new_piplus_energy.GetYaxis()->SetLabelSize(0.06);	
	new_piplus_energy.GetXaxis()->SetLabelSize(0.06);	
    new_piplus_energy.SetFillColorAlpha(kMagenta,0.3);
	new_piplus_energy.SetLineColor(kMagenta+3);
    ...
    for(int i=0; i < tree->GetEntries(); i++) 
    {
        ...
        for(int ip = 0; ip < StdHepN; ip++) 
        {
            ...
            sdy = (Y+sy1) - y0;	                 
            sdz = (Z+sz1) - z0;                   
            /*if (sdz < 0) //considering only the forward half
            {
            ...
            {
                if(StdHepPdg[StdHepFm[ip]] == mu_nu_pdg && StdHepPdg[ip] == mu_pdg)    //mu_nu is mother, hence primary
                {
                    orig_primary_muminus_energy.Fill(energy);
                    orig_primary_muon_count[self]++;	
                }
                else if (StdHepPdg[StdHepFm[ip]] != mu_nu_pdg  && StdHepPdg[ip] == mu_pdg)
                {
			...
			if (StdHepStatus[ip] == 1 && srad_distance > stt_rmax && mrad_distance <= stt_rmax && abs(Z+mz1) < 2569 && abs(Z+mz1) > 2214)  //rock events and mom from STT
			{
				...
				if (StdHepPdg[ip] != StdHepPdg[StdHepFm[ip]] || (ip != StdHepFd[StdHepFm[ip]])) //self != mom particle-wise, or self = mom but self != first daughter of the mom, to ensure that it's not itself that crosses
				{
                    ...
                    { 
                        ...
                        {
                            if(srad_distance >= rock_rmin && srad_distance <= rock_rmax)new_primary_muon_count[self]++;
                        }
                        else if ( StdHepPdg[StdHepFm[ip]] != mu_nu_pdg ) 
                        {
                            if(srad_distance >= rock_rmin && srad_distance <= rock_rmax)new_secondary_muon_count[self]++;
                        }
                    }
                    //mu+ fill
					...
					//counting 
                    //if(srad_distance >= rock_rmin && srad_distance <= rock_rmax && StdHepPdg[StdHepFm[ip]] != piminus_pdg)
                    if(srad_distance >= ecal_rmin && StdHepPdg[StdHepFm[ip]] != StdHepPdg[ip])
                    { 
                        new_particle_count[self]++; 
                        //if (my_pdg.GetName(StdHepPdg[ip]).find("mu") != std::string::npos) std::cout << "New... " << my_pdg.GetName(StdHepPdg[ip]).c_str() << " " << srad_distance << ", Mother: " << my_pdg.GetName(StdHepPdg[StdHepFm[ip]]) << " " << mrad_distance << "\n";
                    }
				}
				if (print2 == "-Q")printf("\n ------------------------------------------------------------------------------------------------------------------------------------------------------------------");
			}
    ...
    printf ("\n vol                            orig prim. mu-       cros. prim. mu-    stuck. prim. mu-   orig. pi-         cros.+new pi-       stuck pi-           orig. pi+           cros.+new pi+       stuck pi+");
    printf ("\n %-30s %-19d %-19d %-19d %-19d %-19d %-19d %-19d %-19d %-19d\n", post_vol.c_str(), orig_primary_muon_count[mu_pdg], crossed_primary_muon_count[mu_pdg], stuck_primary_muon_count[mu_pdg], orig_particle_count[piminus_pdg], crossed_particle_count[piminus_pdg]+new_particle_count[piminus_pdg], stuck_particle_count[piminus_pdg], orig_particle_count[-piminus_pdg], crossed_particle_count[-piminus_pdg]+new_particle_count[-piminus_pdg], stuck_particle_count[-piminus_pdg]);
